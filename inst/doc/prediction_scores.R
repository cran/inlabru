...
## ----eval=FALSE-----------------------------------------------------------------------------------
#  max_K <- ceiling(max(y)) + 4 * sqrt(max(y)) # some large value, so that 1-F(K) is small
...
#       pull(crps))
#  # Check that the cutoff point K has nearly probability mass 1 below it, for all i:
#  min(F_estimate)
...
## ----eval=FALSE-----------------------------------------------------------------------------------
#  pred <- predict(fit, newdata, formula = ~ list(cond_scores = (y - exp(eta))^2,
#                                                 lambda = exp(eta)))
